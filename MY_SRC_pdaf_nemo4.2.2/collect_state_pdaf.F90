...
subroutine collect_state_pdaf(dim_p, state_p)
  ...
  use nemo_pdaf, &
       ...
       jp_tem, jp_sal, ssh, ts, uu, vv, Nnn
#if defined key_top
  use statevector_pdaf, &
       only: n_trc, sv_trc
  use nemo_pdaf, &
...
! *** Local variables ***
  ...
  ! SSH
  if (id%ssh > 0) then
     call field2state(ssh(1+i0:ni_p+i0, 1+j0:nj_p+j0, Nnn), &
          state_p, &
  ...
  ! T
  if (id%temp > 0) then
     call field2state(ts(1+i0:ni_p+i0, 1+j0:nj_p+j0, 1:nk_p, jp_tem, Nnn), &
          state_p, &
  ...
  ! S
  if (id%salt > 0) then
     call field2state(ts(1+i0:ni_p+i0, 1+j0:nj_p+j0, 1:nk_p, jp_sal, Nnn), &
          state_p, &
  ...
  ! U
  if (id%uvel > 0) then
       call field2state(uu(1+i0:ni_p+i0, 1+j0:nj_p+j0, 1:nk_p, Nnn), &
            state_p, &
  ...
  ! V
  if (id%vvel > 0) then
      call field2state(vv(1+i0:ni_p+i0, 1+j0:nj_p+j0, 1:nk_p, Nnn), &
           state_p, &
  ...
  do i = 1, jptra
     if (sv_trc(i)) then
        call field2state(tr(1+i0:ni_p+i0, 1+j0:nj_p+j0, 1:nk_p, sfields(id%trc(i))%id_trc, Nnn), &
             state_p, &
